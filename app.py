...
st.markdown("""
    ...
    /* Contenedor estricto para forzar el mismo tamaño de imagen sin deformar */
    .img-contenedor {
        width: 100%;
        height: 220px; /* Altura fija para que todas queden exactamente iguales */
        overflow: hidden;
        border-radius: 8px;
        margin-bottom: 15px;
    }
    .img-contenedor img {
        width: 100%;
        height: 100%;
        object-fit: cover; /* Recorta la imagen proporcionalmente para llenar el espacio */
        object-position: center top; /* Centra el enfoque en la cara del candidato */
    }
    </style>
...
for cand in candidatos:
    with cand["col"]:
        # Abrimos la tarjeta contenedora blanca
        st.markdown('<div class="candidato-card">', unsafe_allow_html=True)
        
        # Inyectamos la imagen usando el contenedor HTML con tamaño controlado
        if os.path.exists(cand["foto"]):
            ...
            st.markdown(f'<div class="img-contenedor" style="background:#eee; display:flex; align-items:center; justify-content:center; color:#777;">Falta {cand["foto"]}</div>', unsafe_allow_html=True)
            
        # Textos informativos de las tarjetas
        st.markdown(f"<h4 style='color: #003893; margin: 5px 0;'>{cand['nombre']}</h4>", unsafe_allow_html=True)
        st.markdown(f"<p style='color:#333; font-size:0.95rem; margin-bottom:8px;'><b>Línea/Partido:</b><br>{cand['partido']}</p>", unsafe_allow_html=True)
        st.markdown(f"<p style='color:#555; font-size:0.85rem; text-align:justify;'><i>Enfoque:</i> {cand['propuesta']}</p>", unsafe_allow_html=True)
